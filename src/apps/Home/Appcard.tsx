      ...
      <div className="relative z-10 p-4 sm:p-5 h-full flex flex-col justify-between min-h-[200px] sm:min-h-[220px]">
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="space-y-2 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
            <p className="text-xs sm:text-sm text-white/95 line-clamp-2 drop-shadow-md">
              {description}
            </p>
            ...
            <div className="flex flex-wrap gap-1.5">
              {techStack.slice(0, 3).map((tech, index) => (
                <span
                  key={index}
                  className="px-2 py-0.5 sm:px-2.5 sm:py-1 text-xs font-medium rounded-md bg-white/20 border border-white/20 text-white"
                >
                  {tech}
                </span>
              ))}
