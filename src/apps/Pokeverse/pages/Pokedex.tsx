...
import { Grid, Box, CircularProgress, Typography, Container, useTheme, Divider, Button } from '@mui/joy';
import { AlertCircle, Search, ArrowUp, PawPrint } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { Link } from 'react-router-dom';
import PokemonCard from '../components/PokemonCard';
import { BASE_API, REGION_DATA, TYPE_COLORS } from '../helpers/constant';
import { PokemonDetail, PokemonListResponse } from '../helpers/types';
import useCacheApi from '../helpers/useCacheApi';
import { scrollToTop } from '../helpers/utilities';

const LIMIT = 20;
const CACHE_TIME = 15 * 60 * 1000; // 15 min cache for Pokemon data
...
const Pokedex: React.FC = () => {
    ...
    const fetchSinglePokemon = async (url: string): Promise<PokemonDetail | null> => {
        try {
    ...
    const fetchPokemonDetails = async (results: PokemonListResponse['results']) => {
        ...
        try {
            const detailPromises = results.map(pokemon => fetchSinglePokemon(pokemon.url));
            const details = await Promise.all(detailPromises);
            const validDetails = details.filter((detail): detail is PokemonDetail => detail !== null);
            return validDetails;
        } finally {
        ...
        if (data?.results) {
            fetchPokemonDetails(data.results).then(details => {
                if (details && details.length > 0) {
                    ...
                    setHasMore(offset + LIMIT < selectedRegion.endId);
                }
            });
        }
    }, [data]);
