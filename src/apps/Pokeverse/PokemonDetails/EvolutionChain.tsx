...
function EvolutionCard({
    ...
    return (
        <button
            ...
            className={`group relative rounded-xl border-2 transition-all duration-200 ${isCompact ? 'p-2' : 'p-3'
                } ${isCurrent
        ...
        >
            ...
            <div className="space-y-1">
                <p className={`font-medium text-neutral-500 dark:text-neutral-400 ${isCompact ? 'text-xs' : 'text-xs'
                    }`}>
...
// Recursive function to build evolution stages horizontally
function renderHorizontalStage(
    evolution: EvolutionDetails,
    currentId: number,
    depth: number = 0
) {
            ...
            {evolution.evolvesTo.length > 0 && (
                <>
                    ...
                    {hasMultipleEvolutions ? (
                        // Vertical branching for multiple evolutions
                        ...
                        // Continue horizontal for single evolution
...
export default function EvolutionChain({ pokemon }: EvolutionChainProps) {
