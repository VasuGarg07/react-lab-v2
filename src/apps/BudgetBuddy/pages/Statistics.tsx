...
const Tooltip_ = ({ active, payload }: any) => {
    ...
    return (
        <div className="bg-white dark:bg-neutral-800 px-3 py-2 rounded-sm border border-neutral-200 dark:border-neutral-700 text-xs">
            ...
            <p className="text-neutral-500 dark:text-neutral-400">{formatCurrency(value)}</p>
        </div>
    );
};
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-neutral-800 rounded-sm p-5 border border-neutral-200 dark:border-neutral-700">
                <SectionLabel title="Income vs Expense" />
                <div className="flex flex-col sm:flex-row items-center gap-8">
                    <div className="w-full sm:w-48 shrink-0">
                        <ResponsiveContainer width="100%" height={180}>
                            <PieChart>
                                <Pie data={incomeExpenseData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={2} dataKey="value">
                    ...
                    <div className="flex sm:flex-col gap-8 sm:gap-5">
                        <div>
                            <p className="text-xs text-neutral-400 dark:text-neutral-500 uppercase tracking-widest mb-1">Income</p>
                            <p className="text-xl font-medium tracking-tight text-emerald-600 dark:text-emerald-400">
                                {total > 0 ? ((totalIncome / total) * 100).toFixed(1) : 0}%
                            </p>
                            <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">{formatCurrency(totalIncome)}</p>
                        </div>
                        <div>
                            <p className="text-xs text-neutral-400 dark:text-neutral-500 uppercase tracking-widest mb-1">Expense</p>
                            <p className="text-xl font-medium tracking-tight text-red-600 dark:text-red-400">
                                {total > 0 ? ((totalExpense / total) * 100).toFixed(1) : 0}%
                            </p>
                            <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">{formatCurrency(totalExpense)}</p>
                        </div>
                    </div>
                </div>
            </div>
