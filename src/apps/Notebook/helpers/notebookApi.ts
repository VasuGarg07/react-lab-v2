...
export const getNotebookById = (id: string): Promise<{ data: { notebook: Notebook; chapters?: NotebookPage[] } }> =>
...
export const exportNotebookAsJson = (id: string): Promise<{ data: { notebook: Notebook; chapters: NotebookPage[] } }> =>
...
export const getChaptersForNotebook = (notebookId: string): Promise<{ data: NotebookPage[] }> =>
    apiClient.get(`/notebook/chapters/${notebookId}`);

export const searchChaptersInNotebook = (notebookId: string, query: string): Promise<{ data: NotebookPage[] }> =>
    apiClient.get(`/notebook/chapter/search/${notebookId}/${query}`);

export const getChapterById = (id: string): Promise<{ data: { chapter: NotebookPage } }> =>
    apiClient.get(`/notebook/chapter/${id}`);

export const createChapter = (body: CreateChapterRequest): Promise<{ data: { chapter: NotebookPage } }> =>
    apiClient.post("/notebook/chapter/create", body);

export const updateChapter = (id: string, body: UpdateChapterRequest): Promise<{ data: { chapter: NotebookPage } }> =>
