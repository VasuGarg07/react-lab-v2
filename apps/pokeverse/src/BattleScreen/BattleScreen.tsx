                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                        <div className="order-2 md:order-1">
                            <TeamPartyStatus team={opponent.team} playerName={opponent.name} isOpponent={true} />
                        </div>
                        <div className="order-1 md:order-2">
                            <PokemonDisplay pokemon={opponentPokemon} playerName={opponent.name} isOpponent={true} />
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                        <div className="order-1">
                            <PokemonDisplay pokemon={activePokemon} playerName={currentPlayer.name} isOpponent={false} />
                        </div>
                        <div className="order-2">
                            <TeamPartyStatus team={currentPlayer.team} playerName={currentPlayer.name} isOpponent={false} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        <div className="lg:col-span-1 order-1 lg:order-2">
                            {!currentPlayer.hasActed ? (
                                selectedAction === null ? (
                                    <ActionPanel onFight={() => setSelectedAction('fight')} onSwitch={() => setSelectedAction('switch')} onForfeit={handleForfeit} />
