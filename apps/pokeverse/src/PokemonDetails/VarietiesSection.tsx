    ...
    if (!pokemon.varieties || pokemon.varieties.length === 0) {
        return (
            <div className="p-6 bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 text-center">
                ...
                <p className="text-sm text-neutral-600 dark:text-neutral-400">No alternate forms available</p>
            </div>
        );
    }
            ...
            {pokemon.varieties.map((variety) => (
                ...
                >
                    ...
                    <div className="space-y-1">
                        <p className="text-xs font-medium text-neutral-500 dark:text-neutral-400">{formatPokemonId(variety.id)}</p>
                        <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 capitalize truncate">{formatString(variety.name)}</p>
                    </div>
                </button>
            ))}
        </div>
