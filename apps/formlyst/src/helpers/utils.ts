...
import type {
    ...
    FormConfig, FormStep, FormSection, FormField,
    BreadcrumbItem, TreeNode, FormResponse,
} from './types';
...
export function createStep(title: string, existingKeys: string[]): FormStep {
    return { key: generateKey(title, 'step', existingKeys), title, description: '', sections: [] };
}

export function createSection(title: string, existingKeys: string[]): FormSection {
    return { key: generateKey(title, 'section', existingKeys), title, description: '', fields: [] };
}

export function createField(type: FieldType, label: string, existingKeys: string[]): FormField {
    const base = { key: generateKey(label, 'field', existingKeys), label, required: false };
    switch (type) {
        case 'text': return { ...base, type: 'text' };
        case 'number': return { ...base, type: 'number' };
        case 'select': return { ...base, type: 'select', options: [] };
        case 'multi_select': return { ...base, type: 'multi_select', options: [] };
        case 'boolean': return { ...base, type: 'boolean' };
        case 'range': return { ...base, type: 'range', min: LIMITS.RANGE_MIN, max: LIMITS.RANGE_MAX };
    }
}
...
export function buildBreadcrumbs(config: FormConfig, path: string[]): BreadcrumbItem[] {
    ...
    if (step) crumbs.push({ key: step.key, label: step.title, type: 'step', path: [step.key] });
    if (path.length >= 2 && step) {
        const section = step.sections.find((s) => s.key === path[1]);
        if (section) crumbs.push({ key: section.key, label: section.title, type: 'section', path: [path[0], section.key] });
    }
    if (path.length >= 3 && step) {
        const section = step.sections.find((s) => s.key === path[1]);
        const field = section?.fields.find((f) => f.key === path[2]);
        if (field) crumbs.push({ key: field.key, label: field.label, type: 'field', path: [path[0], path[1], field.key] });
    }
    return crumbs;
}
...
export function buildTree(config: FormConfig): TreeNode[] {
    return [{
        key: 'form', label: config.title || 'New Form', type: 'form',
        children: config.steps.map((step) => ({
            key: step.key, label: step.title, type: 'step' as const,
            children: step.sections.map((section) => ({
                key: section.key, label: section.title, type: 'section' as const,
                children: section.fields.map((field) => ({
                    key: field.key, label: field.label, type: 'field' as const,
                })),
            })),
        })),
    }];
