    ...
    return (
        <aside className="w-64 shrink-0 bg-white dark:bg-neutral-800 border-r border-neutral-200 dark:border-neutral-700 flex flex-col h-full">
            <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200 dark:border-neutral-700">
                <h2 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">Structure</h2>
                <button onClick={onToggle} className="p-1.5 rounded-md text-neutral-500 hover:text-neutral-700 dark:text-neutral-400 dark:hover:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors" title="Close sidebar">
            ...
            <div className="px-4 py-3 border-t border-neutral-200 dark:border-neutral-700">
                ...
                <div className="h-1.5 bg-neutral-100 dark:bg-neutral-700 rounded-full overflow-hidden">
                    <div
                        className={`h-full rounded-full transition-all duration-300 ${fieldPercentage > 80 ? 'bg-amber-500' : fieldPercentage > 95 ? 'bg-red-500' : 'bg-blue-500'}`}
                        style={{ width: `${Math.min(fieldPercentage, 100)}%` }}
                    />
                </div>
                <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1 text-center">{fieldPercentage}% capacity used</p>
            </div>
        </aside>
    );
}
