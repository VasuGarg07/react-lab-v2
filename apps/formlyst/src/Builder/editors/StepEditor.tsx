...
export default function StepEditor({ stepKey }: StepEditorProps) {
    ...
    useEffect(() => {
        if (step) { setLocalTitle(step.title); setLocalDescription(step.description || ''); }
    }, [step?.key]);
    ...
    const handleDescriptionBlur = () => {
        if (localDescription !== (step.description || '')) dispatch(updateStep({ key: stepKey, description: localDescription }));
    };

    const handleAddSection = () => modal.open(<AddEntityDialog entityType="section" onAdd={(title) => dispatch(addSection({ stepKey, title }))} />);
            ...
            <div>
                ...
                {step.sections.length > 0 ? (
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        <SortableContext items={step.sections.map((s) => s.key)} strategy={verticalListSortingStrategy}>
                            <div className="space-y-2">
                                {step.sections.map((section) => (
                                    <SortableEntityCard
                                        key={section.key} id={section.key} type="section" title={section.title}
