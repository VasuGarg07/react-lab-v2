...
import {
    ...
    markCardsAsMatched, resetTurn, setGameState,
...
export function CardGrid() {
    ...
    useEffect(() => {
        ...
        if (firstChoice.matchingCardId === secondChoice.id) {
            dispatch(markCardsAsMatched({ firstId: firstChoice.id, secondId: secondChoice.id }));
            dispatch(resetTurn());
        } else {
    ...
    useEffect(() => {
        if (cards.length > 0 && cards.every((c) => c.matched)) {
            setTimeout(() => dispatch(setGameState('game_end')), 500);
        }
    }, [cards, dispatch]);
