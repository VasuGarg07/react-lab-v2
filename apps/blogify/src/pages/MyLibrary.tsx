        ...
        <div className="max-w-4xl mx-auto space-y-8">
            <header className="flex items-center justify-between pt-2">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <h1 className="font-serif text-xl text-stone-900 dark:text-stone-100 leading-tight">{user?.username}</h1>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        onClick={() => setShowSettings(!showSettings)}
                        className={`p-2 rounded-xl transition-colors ${showSettings ? 'bg-stone-200 dark:bg-stone-700 text-stone-900 dark:text-stone-100' : 'bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400 hover:bg-stone-200 dark:hover:bg-stone-700'}`}
                    >
            ...
            {showSettings && (
                <div className="p-4 rounded-2xl bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 space-y-2">
                    <p className="text-xs font-medium text-stone-400 dark:text-stone-500 uppercase tracking-wide mb-3">Bulk actions</p>
                    <button
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="text-xs text-stone-400 dark:text-stone-500 mt-0.5">Remove {draftCount} {draftCount === 1 ? 'draft' : 'drafts'}</p>
                        </div>
                    </button>
                    <button
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="text-xs text-red-400 dark:text-red-500 mt-0.5">Permanently remove all {blogCount} {blogCount === 1 ? 'blog' : 'blogs'}</p>
                        </div>
                    </button>
                </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-3">
                {[
                    ...
                    <div key={label} className="py-5 px-4 rounded-2xl bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 text-center">
